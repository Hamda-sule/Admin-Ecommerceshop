...
//creating end point registrarion
...
addAbortListener.get('/newcollections', async (req, res) => {
    let products = await Product.find({});
...
// Middleware to user
 const fetch = async (req, res, next)=>{
    ...
    if (!token) {
        res.status(401).send({errors:"Please authentcated using vslid"})
    }
    else {
        try {
...
app.get('/addtocart', fetchUser,  async (res, req)=> {
...
app.post('/removefromfcart', fetchUser, async (req, res)=> {
